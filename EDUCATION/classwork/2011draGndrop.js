"use strict";
//let klubnika = document.querySelector(".klubnika");
//klubnika.style.position = "absolute";
//klubnika.style.top = 0;
//klubnika.style.left = 0;
//let counter = 0;
//let interval = setInterval(() => {
//  counter += 5;
//  klubnika.style.top = counter + "px";
//  klubnika.style.left = counter + "px";
//  if (counter > 500) {
...
function dragItem(event) {
    ...
    let item = this; //event.currentTarget
    item.style.position = "absolute";
    item.style.zIndex = '99999';//делаем элемент поверх остальных
    ...
    //    //Элемент по центру
    //    item.style.top = (event.clientY - item.getBoundingClientRect().height/2) + "px"; // event.clientY - текущая позиция по оси Y
    //    item.style.left = (event.clientX - item.getBoundingClientRect().width/2) + "px";
