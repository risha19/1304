...
//2.Через св-во элемента. Если событие стандартное, к нему можно обратиться через св-во
////таким способом можно повесить только одну ф-ю
...
//propertyButton2.onclick = chowAdvert; //передаем тело ф-и
//function chowAdvert() {
...
//
//
////3. Через метод addEventListener(event, handler)
////позволяет повесить несколько обработчиков на одно и то же собитие
////позволяет работать с нестандатными собитиями
//
...
//});
////удаляем события через removeEventListener. Для работоспособности ф-я -обработчик должна быть именованной
//function handler3() {
...
//  eventButton.removeEventListener("click", handler3);//удаляем слушателя событий
//  //название удаляемой функции-обработчика должно совпадать
//});
//
